...
class RegisterViewModel: ViewModel() {
    ...
    // To notify the current fragment to navigate to the activity
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                if (password != repeatPassword) throw PasswordNoMatchException(
                    App.applicationContext().getString(R.string.password_no_match)
                )
    ...
    private fun parseDateFromDatePicker(birthDateView: DatePicker): String {
        ...
        if (birthDateView.month + 1 <= 9) parsedDate += "0"
        parsedDate += birthDateView.month + 1
        parsedDate += "-"

        if (birthDateView.dayOfMonth <= 9) parsedDate += "0"
